    ...
    <div className="p-4 md:p-6 bg-gray-50 min-h-screen">
        ...
        <div className="overflow-x-auto bg-white shadow-xl rounded-xl border border-gray-200">
          <table className="w-full border-collapse text-sm min-w-full responsive-table">
            <thead className="bg-gradient-to-r from-[#791010] to-[#5a0c0c] text-white">
              <tr>
                <th className="px-3 md:px-4 py-2 md:py-3 text-left font-semibold w-16 md:w-20" data-label="School ID">School ID</th>
                <th className="px-3 md:px-4 py-2 md:py-3 text-left font-semibold min-w-24 md:min-w-32" data-label="Full Name">Full Name</th>
                <th className="px-3 md:px-4 py-2 md:py-3 text-left font-semibold min-w-24 md:min-w-32" data-label="Email">Email</th>
                <th className="px-3 md:px-4 py-2 md:py-3 text-left font-semibold hidden md:table-cell w-16 md:w-20" data-label="Course">Course</th>
                <th className="px-3 md:px-4 py-2 md:py-3 text-left font-semibold hidden lg:table-cell w-12 md:w-16" data-label="Year">Year</th>
                <th className="px-3 md:px-4 py-2 md:py-3 text-center font-semibold w-20 md:w-24" data-label="Approval">Approval</th>
                <th className="px-3 md:px-4 py-2 md:py-3 text-center font-semibold w-20 md:w-24" data-label="Status">Status</th>
                <th className="px-3 md:px-4 py-2 md:py-3 text-center font-semibold w-24 md:w-28" data-label="Actions">Actions</th>
              </tr>
            </thead>
            <tbody>
              {filteredVoters.map((voter, idx) => (
                ...
                >
                  <td className="px-3 py-2 font-medium text-gray-800 whitespace-nowrap" data-label="School ID">{voter.school_id ?? "-"}</td>
                  <td className="px-3 py-2 font-medium text-gray-900 whitespace-nowrap" data-label="Full Name">
                    {voter.fullname}
                  </td>
                  <td className="px-3 py-2 text-gray-700 whitespace-nowrap" data-label="Email">{voter.email}</td>
                  <td className="px-3 py-2 text-gray-700 hidden md:table-cell whitespace-nowrap" data-label="Course">
                    {voter.course ?? "-"}
                  </td>
                  <td className="px-3 py-2 text-gray-700 hidden lg:table-cell whitespace-nowrap" data-label="Year">
                    {voter.year_level ?? "-"}
                  </td>

                  {/* Approval Status */}
                  <td className="px-3 py-2 text-center align-middle" data-label="Approval">
                    {voter.approval_status === "approved" ? (
                      <span className="bg-green-100 text-green-800 px-2 py-1 rounded-full text-xs font-bold whitespace-nowrap inline-flex items-center">
                        <span className="mr-1">✅</span>
                        <span className="hidden md:inline">Approved</span>
                      </span>
                    ) : voter.approval_status === "declined" ? (
                      <span className="bg-red-100 text-red-800 px-2 py-1 rounded-full text-xs font-bold whitespace-nowrap inline-flex items-center">
                        <span className="mr-1">❌</span>
                        <span className="hidden md:inline">Declined</span>
                      </span>
                    ) : (
                      <span className="bg-yellow-100 text-yellow-800 px-2 py-1 rounded-full text-xs font-bold whitespace-nowrap inline-flex items-center">
                        <span className="mr-1">⏳</span>
                  ...
                  {/* User Status */}
                  <td className="px-3 py-2 text-center align-middle" data-label="Status">
                    {voter.user_status === "active" ? (
                      <span className="bg-green-50 text-green-800 px-2 py-1 rounded-full text-xs font-bold whitespace-nowrap inline-flex items-center">
                        <span className="mr-1">🔵</span>
                        <span className="hidden md:inline">Active</span>
                      </span>
                    ) : voter.user_status === "inactive" ? (
                      <span className="bg-gray-200 text-gray-800 px-2 py-1 rounded-full text-xs font-bold whitespace-nowrap inline-flex items-center">
                        <span className="mr-1">⚪</span>
                        <span className="hidden md:inline">Inactive</span>
                      </span>
                    ) : (
                      <span className="bg-purple-50 text-purple-800 px-2 py-1 rounded-full text-xs font-bold whitespace-nowrap inline-flex items-center">
                        <span className="mr-1">🎓</span>
                  ...
                  {/* Actions */}
                  <td className="px-3 py-2 text-center align-middle" data-label="Actions">
                    {(!voter.approval_status ||
                      voter.approval_status === "pending") && (
                      <div className="flex flex-col sm:flex-row gap-1 justify-center mb-2">
                        <button
                          onClick={() =>
                        ...
                        <button
                          ...
                          className="px-2 py-1 md:px-3 md:py-1.5 bg-red-600 text-white rounded-lg shadow hover:bg-red-700 transition-all duration-200 text-xs font-medium flex items-center justify-center gap-1"
                        >
      ...
      {confirmAction.voter && (
        <div className="modal-overlay flex items-center justify-center">
          ...
          {/* Modal */}
          <div className="modal-content relative bg-white p-6 rounded-2xl shadow-2xl max-w-md w-full mx-4 border border-gray-200">
      ...
      {editModal.isOpen && editModal.voter && (
        <div className="modal-overlay flex items-center justify-center">
          ...
          {/* Modal */}
          <div className="modal-content relative bg-white p-6 rounded-2xl shadow-2xl max-w-lg w-full mx-4 border border-gray-200 max-h-[90vh] overflow-y-auto">
            <div className="text-center mb-4">
